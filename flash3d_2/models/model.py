...
class GaussianPredictor(nn.Module):
    ...
    def target_frame_ids(self, inputs):
        return inputs["target_frame_ids"] #novel view frame_ids 
    
    def all_frame_ids(self, inputs):
        return add_source_frame_id(self.target_frame_ids(inputs)) #List에 novel view frame_ids가 존재함 
    ...
    def load_ply(self, path, requires_grad: bool = True): #gaussian_model.py에서 참고 
    ...
    def compute_gauss_means(self, inputs, outputs):
        ...
        xyz = self.backproject_depth[str(scale)](depth, inv_K) # 해당 방법을 아에 visual hull을 사용해 얻은 xyz (ply파일로 얻게된 것)로 대체해서 아래 과정을 실행 
        # xyz = load_ply(cfg) #args값은 cfg에 ply를 저장한 위치를 지정해놓은 것을 사용하는 코드 작성 필요 
    ...
    def render_images(self, inputs, outputs):
        ...
        for scale in [0]: #cfg.model.scales:
            ...
            frame_ids = self.all_frame_ids(inputs) # novel view frame_ids [0, novel view frame ids]
            ...
            for frame_id in frame_ids:
                if frame_id == 0:
                    ...
                    if ('cam_T_cam', 0, frame_id) not in outputs:
                        continue
                    T = outputs[('cam_T_cam', 0, frame_id)] #0 frames에서 frame_id로의 변환 행렬 
                ...
                for b in range(B):
                    ...
                    #pc 에는 xyz, opacity, rotation, scaling, feature_dc 값이 들어있음

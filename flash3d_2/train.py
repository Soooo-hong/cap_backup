...
def run_epoch(fabric,
    ...
    for batch_idx, inputs in enumerate(train_loader):
        ...
        if fabric.is_global_zero:
            ...
            # save the model
            if (trainer.epoch % 5 == 0) and (trainer.epoch != 0):  # Save every 5 epochs
                trainer.model.save_model(optimiser, trainer.epoch, ema)
...
def main(cfg: DictConfig):
    ...
    train_loader = fabric.setup_dataloaders(train_loader, move_to_device=True)
    ...
    for trainer.epoch in tqdm(range(cfg.optimiser.num_epochs), desc="Training Epochs", unit="epoch"):

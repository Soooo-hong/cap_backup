...
from GaussianObject.gaussian_renderer import render

TENSORBOARD_FOUND = True

def training(cfg, fine_visual_hull,image_name,image_idx,dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from) : 
    first_iter = 0
    dataset.model_path = f'/home/shk00315/capston2/flash3d/data/omni3d/{image_name}' # 이건 나중에 모든 데이터셋에 접근 가능하도록 변경해야됨 
    tb_writer = prepare_output_and_logger(dataset)
    gaussians = GaussianModel(dataset.sh_degree)
    scene = Scene(fine_visual_hull,image_name,dataset,gaussians,extra_opts=cfg)
    gaussians.training_setup(opt)
    # gaussians.load_ply(os.path.join('/home/shk00315/cap_2/flash3d_2/result_images/initial_gaussian_with_visual_hull','input.ply'))
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        while network_gui.conn != None:
            try:
                ...
                network_gui.send(net_image_bytes,fine_visual_hull) # 바꿈 
        ...
        viewpoint_cam = viewpoint_stack[image_idx]
        # Render
        ...
        # Loss
        loss, Ll1 = cal_loss(opt, cfg, image, render_pkg, viewpoint_cam, bg, tb_writer=tb_writer, iteration=iteration, mono_loss_type=cfg.mono_loss_type)
...
def visual_hull_training(cfg,fine_visual_hull,image_name,image_idxs) : 
    lp = ModelParams(cfg)
    ...
    for visual_hulls in fine_visual_hull.values() :  #visual_hulls = ((b1_points,b2_points),(b1_colors,b2_colors))
        for idx in range(len(visual_hulls)) :
            ...
            image_idx = image_idxs[idx]
            loss = training(cfg, visual_hull,image_name,image_idx,lp.extract(cfg), op.extract(cfg), pp.extract(cfg), cfg.test_iterations, 
